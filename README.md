...
Window metadata comes from the published [Pearson External Assessment Overview](https://pearsontablebuilder.netlify.app/ld/btec-external-assessment-overview-8u07), checked on 17 September 2026. `pnpm sync:windows` reads only its published Winter/Summer 2027 timetable sources and writes `src/data/exam-windows.json`. It uses the public anonymous read interface loaded by Pearson's page; if that interface changes, the command fails for maintainer review. No API credential is stored. An optional local JSON snapshot with `sources` and `rows` can be passed for offline preparation.

Matching requires exactly one event with the same code, qualification, subject, title, date, unit, session and duration. All 32 existing Window events matched. Ambiguous/unmatched events or reversed ranges stop the sync before any output is written. Metadata is keyed by the curated event ID so a later timetable import cannot attach an old window to a changed event. Review the generated diff, then run tests/build and publish. The live app uses the checked-in snapshot; it does not continuously sync or send booking details to Pearson.

Selecting a window assessment opens its date range, part, material release, deadline and source notes. The information remains on the form and in event details. Staff must choose a date within the inclusive range; missing bounds block generation until the data is confirmed. Pre-release/preparation events remain explicitly labelled. The chosen booking date is used in the Word form and filename; the window and submission deadline are added to booking notes. Fixed-date exams retain their timetable date. Changing assessments clears the selected booking date. Coming up retains an open window until its end date. The month grid continues to mark each event's original timetable date; opening it shows the full window.

Validation: 35 tests pass, including source matching, malformed/reversed dates, separate parts, both window boundaries, single-day windows, missing data, expiry, date reset and the actual generated Word document. Browser checks cover the automatic pop-up, keyboard date selection and booking review. Word pagination remains subject to the existing visual-verification limitation above.

The booking selector includes an `On-demand exams only` checkbox. It filters on an explicit `onDemand: true`; absent values remain scheduled. No on-demand records are currently supplied, so the filter shows an explanatory empty state. Import of the proposed `On Demand?` column and date-free on-demand booking will be completed with the first sample records; the current importer still expects scheduled dates. No Supabase instance or authentication has been configured.

Window bookings support one or more sessions, each with a date and start/end time. Every session must be within the published window, end after it starts, and avoid overlap with other sessions. Combined hours are shown alongside the timetable duration for staff to check. Session numbers are local scheduling labels, not Pearson part labels. The single Word booking lists all dates and the full session schedule in its notes; its filename uses the earliest session date. Changing assessments resets the sessions. Request date remains separate. The 38 tests include multi-session Word output and timing validation.
